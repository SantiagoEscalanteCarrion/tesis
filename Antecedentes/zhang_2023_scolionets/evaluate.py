"""
Evaluación K-Fold + Robustez — Zhang et al. (2023) ScolioNets
==============================================================
...
def _make_ds(paths_arr, labels_arr, pixel_sigma=0.0, shuffle=False, seed=SEED):
    """
    Dataset con preprocesamiento ResNet50.
...
def evaluate_zhang2023(dataset_dir, output_dir, k=5,
                       epochs_head=5, epochs_fine=10, seed=SEED):
    """
    Por cada fold:
      1. Entrena ScolioNets (Fase 1 cabeza + Fase 2 fine-tuning)
    ...
    print("\n" + "█"*60)
    print("  Zhang et al. (2023) — ScolioNets")
    print("  ResNet50 + Attention Branch Network")
    ...
    fold_metrics   = []                            # métricas limpias por fold
    noise_results  = {s: [] for s in NOISE_LEVELS} # métricas por nivel de ruido
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(
        ...
        model.fit(train_ds, epochs=epochs_head, verbose=1,
...
def _plot_kfold(fold_metrics, output_dir, k):
    ...
    print(f"\n  {'═'*55}")
    print(f"  K-FOLD CV (k={k}) — Zhang et al. (2023) ScolioNets")
...
def _plot_robustness(noise_results, output_dir):
    ...
    print(f"\n  {'═'*55}")
    print(f"  ROBUSTEZ — Zhang et al. (2023) ScolioNets")
    ...
    fig.suptitle(
        f"Robustez bajo Ruido Gaussiano — Zhang et al. (2023) ScolioNets\n"
        f"Accuracy limpia: {mean_accs[0]:.3f} → máximo ruido (σ=0.5): {mean_accs[-1]:.3f}",
        fontsize=11, fontweight="bold", y=1.02
    )
    plt.tight_layout()
    path = os.path.join(output_dir, "robustness_zhang2023.png")
    fig.savefig(path, dpi=150, bbox_inches="tight")
