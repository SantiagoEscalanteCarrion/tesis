"""
Evaluación K-Fold + Robustez — Yang et al. (2019) ResNet-101
=============================================================
...
Uso:
    python evaluate.py
    python evaluate.py --k 5 --epochs_head 5 --epochs_fine 10
"""
...
from model import build_yang2019_resnet101
...
def _make_ds(paths_arr, labels_arr, pixel_sigma=0.0, shuffle=False, seed=SEED):
    """
    Dataset con preprocesamiento ResNet (mismo que ResNet50).
    pixel_sigma > 0 aplica ruido gaussiano: N(0, sigma × 255).
    """
...
def evaluate_yang2019(dataset_dir, output_dir, k=5,
                      epochs_head=5, epochs_fine=10, seed=SEED):
    """
    Por cada fold:
      1. Fase 1: entrena cabeza (backbone frozen)
      2. Fase 2: fine-tuning bloque conv5 de ResNet-101
      3. Evalúa en test limpio  → métricas K-Fold
      4. Evalúa en test ruidoso → curva de robustez
    """
    out = os.path.join(output_dir, "yang_2019_resnet101")
    os.makedirs(out, exist_ok=True)

    print("\n" + "█"*60)
    print("  Yang et al. (2019) — ResNet-101")
    print("  Faster R-CNN + ResNet-101 (Faster R-CNN omitido)")
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(
        ...
        model = build_yang2019_resnet101(trainable_base=False)
        model.compile(
        ...
        model.fit(
            train_ds, epochs=epochs_head, verbose=1,
            callbacks=[tf.keras.callbacks.EarlyStopping(
                monitor="loss", patience=3, restore_best_weights=True)]
        )

        # ── Fase 2: fine-tuning conv5 ─────────────────────────────────
        print("    Fase 2 — fine-tuning conv5_block...")
        backbone = model.get_layer("resnet101")
        ...
        model.fit(
            train_ds, epochs=epochs_fine, verbose=1,
            callbacks=[tf.keras.callbacks.EarlyStopping(
                monitor="loss", patience=3, restore_best_weights=True)]
        )
...
def _plot_kfold(fold_metrics, output_dir, k):
    ...
    print(f"\n  {'═'*55}")
    print(f"  K-FOLD CV (k={k}) — Yang et al. (2019) ResNet-101")
    ...
    fig.suptitle(
        f"5-Fold CV — Yang et al. (2019) ResNet-101\n"
    ...
    plt.tight_layout()
    path = os.path.join(output_dir, "kfold_yang2019.png")
...
def _plot_robustness(noise_results, output_dir):
    ...
    print(f"\n  {'═'*55}")
    print(f"  ROBUSTEZ — Yang et al. (2019) ResNet-101")
    ...
    fig.suptitle(
        f"Robustez bajo Ruido Gaussiano — Yang et al. (2019) ResNet-101\n"
        f"Accuracy limpia: {mean_accs[0]:.3f} → máximo ruido (σ=0.5): {mean_accs[-1]:.3f}",
        fontsize=11, fontweight="bold", y=1.02
    )
    plt.tight_layout()
    path = os.path.join(output_dir, "robustness_yang2019.png")
    fig.savefig(path, dpi=150, bbox_inches="tight")
